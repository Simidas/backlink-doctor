...
| Cloudflare R2 | 100MB 存储 | **$0** | 不存页面快照，只存文本/JSON |
...
**1. DataForSEO 从 $150 → $75**
- 用 `Backlinks` 端点一次性拉取全量外链，非 `OnPage` 逐页爬取
- 本地缓存 7 天，不重复查询同一域名
...
- Playwright 爬虫仅用于：
  - 免费健康检查工具的快照（给用户体验）
  - 偶尔验证 DataForSEO 数据准确性
- 频率降至 500 次/月，用服务器直连即可，无需代理

**4. VPS 从 $10 → $5**
- Hetzner CX11（1 核 1GB，德国区）：$4.51/月
- 只跑轻量 Playwright 脚本，1GB 内存够用

**5. 邮件从 $20+ → $0**
- Resend 免费额度：3000 封/天，MVP 告警邮件 < 1000 封/月
...
- OpenAI：诊断/邮件调用量线性增长
- Cloudflare Workers：超免费额度后 $0.50/百万请求
- 可能需要升级 D1/Queues 付费档

### 8.4 极端省钱方案（如果 $150 还是高）

如果前期真的只有 5-10 个测试用户，可以压到 **$50/月**：
